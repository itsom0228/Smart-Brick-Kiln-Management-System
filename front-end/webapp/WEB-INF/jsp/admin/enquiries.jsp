...
<head>
    ...
    <title>Customer Enquiries - Smart Brick Kiln</title>
...
<div class="admin-wrapper">
    ...
    <div class="admin-sidebar d-flex flex-column justify-content-between">
        <div>
            ...
            <div class="list-group list-group-flush">
                ...
                <a href="${pageContext.request.contextPath}/admin/enquiries" class="list-group-item active"><i class="fa-solid fa-envelope-open-text me-2"></i>Enquiries Inbox</a>
    ...
    <div class="admin-main">
        <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
            <h2 class="text-secondary-custom font-weight-bold mb-0">Customer Enquiries Inbox</h2>
            <div class="text-muted small">Respond to contact form submissions</div>
        </div>
                ...
                <table class="table align-middle table-hover">
                    ...
                    <tbody>
                        <c:forEach var="e" items="${enquiries}">
                            <tr class="${e.replied ? 'opacity-75' : ''}">
                                <td>${e.formattedDate}</td>
                                <td><strong>${e.fullName}</strong></td>
                                <td>${e.mobileNumber}</td>
                                <td>${e.email != null && !e.email.isEmpty() ? e.email : 'N/A'}</td>
                                <td>"${e.messageText}"</td>
                                <td>
                                    <span class="badge ${e.replied ? 'bg-success' : 'bg-danger'}">
                                        ${e.replied ? 'Responded' : 'New Enquiry'}
                                    </span>
                                </td>
                                <td>
                                    <c:if test="${!e.replied}">
                                        <form action="${pageContext.request.contextPath}/admin/enquiries/reply/${e.id}" method="post">
                                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                                            <button type="submit" class="btn btn-sm btn-dark"><i class="fa-solid fa-reply me-1"></i> Mark Responded</button>
                                        </form>
                                    </c:if>
                                    <c:if test="${e.replied}">
                        ...
                        <c:if test="${empty enquiries}">
                            <tr>
                                <td colspan="7" class="text-center text-muted py-5">No customer enquiries received.</td>
                            </tr>
                        </c:if>
                    </tbody>
                </table>
